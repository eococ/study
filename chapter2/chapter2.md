...
- 지역변수 / 전역변수
	...
	(1) **전역변수**는 함수 바깥에 선언하여 클래스 전체에서 사용가능한 변수이다. <br>
	  **지역변수**는 함수 속에 선언하여 해당 함수 속에서만 사용가능한 변수이다. <br>	
	(2) 전역변수의 경우 그 내에서도 (1)클래스 변수와 (2)인스턴스 변수로 나뉜다. 
	<br><br>
...
(1) 인스턴스 변수
	클래스 영역에 선언되고, 인스턴스(객체)를 생성할 때 만들어진다.
	그렇기 때문에 인스턴스의 변수 값을 가져오기 위해선 일단 인스턴스를 생성해줘야 한다.
...
(2) 클래스 변수
	...
	__인스턴스 변수는 인스턴스가 생성될 때마다 생성되므로 인스턴스마다 각기 다른 값을 유지할 수 있지만,__ __클래스 변수는 모든 인스턴스가 하나의__ <br>
